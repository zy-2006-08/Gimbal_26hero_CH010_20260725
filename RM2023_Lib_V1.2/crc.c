/*
 * 统一 CRC 模块实现 (crc-gongban)
 *
 * 4 种算法的查表数值、初值、查表方向、收尾逻辑均从原实现逐字节照抄。
 * A 表(crc_itu16_tab)与 B 表(crc_dji16_tab)数值相同但各留一份，
 * 因为 A 收尾取反、B 不取反，共用会导致误用收尾逻辑。
 */

#include "crc.h"

/* ==================== 算法 A: CRC-ITU16 ==================== */
/* 源: RM_Lib.cpp crctab16 (逐字节照抄) */
static const uint16_t crc_itu16_tab[256] =
{
...
uint16_t crc_itu16(const unsigned char *pData, uint16_t nLength)
{
    ...
    while (nLength > 0)
    {
        fcs = (fcs >> 8) ^ crc_itu16_tab[(fcs ^ *pData) & 0xff];
        nLength--;
        pData++;
    }
    return ~fcs;    /* 取反 */
...
bool crc_itu16_verify(const unsigned char *pData, uint16_t nLength)
{
    ...
    return (fcs == 0xf0b8);  /* 0xf0b8 是 CRC-ITU 的 "Magic Value" */
}

/* ==================== 算法 B: DJI-CRC16 ==================== */
/* 源: CP_System.c wCRC_Table (逐字节照抄) */
static const uint16_t crc_dji16_tab[256] = {
    0x0000, 0x1189, 0x2312, 0x329b, 0x4624, 0x57ad, 0x6536, 0x74bf,
...
uint16_t crc_dji16(const uint8_t *pchMessage, uint32_t dwLength)
{
    ...
    {
        chData = *pchMessage++;
        (wCRC) = ((uint16_t)(wCRC) >> 8) ^ crc_dji16_tab[((uint16_t)(wCRC) ^ (uint16_t)(chData)) & 0x00ff];
    }
    return wCRC;
}

/* ==================== 算法 C: DJI-CRC8 ==================== */
/* 源: CP_System.c CRC8_TAB (逐字节照抄) */
static const unsigned char crc_dji8_tab[256] = { 0x00, 0x5e, 0xbc, 0xe2, 0x61, 0x3f, 0xdd, 0x83, 0xc2,
...
uint8_t crc_dji8(const uint8_t *pchMessage, uint32_t dwLength)
{
    ...
    while (dwLength--)
    {
        ...
        ucCRC8 = crc_dji8_tab[ucIndex];
    }
    return (ucCRC8);
}

/* ==================== 算法 D: Mini_PC 旧 CRC8 ==================== */
/* 源: communication.h crc_table (逐字节照抄) */
static const unsigned char crc_minipc8_tab[] =
{
